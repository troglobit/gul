 ...
 */
void
screenPluginGetMaxYX (int *maxY, int *maxX)
{
   static int init = 1;
   ...
   else
   {
      ...
      *maxX = SLtt_Screen_Cols;
      ...
      if (SCREEN_RESIZED(*maxY, *maxX))
      {
         ...
         {
            LOG(
                     "S-Lang could not properly reallocate the screen. Sorry!\n");
            exit (-1);
         }
      }
   }
